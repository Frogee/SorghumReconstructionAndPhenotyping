...
#############################################################################################
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>#
phenotype_suffix = c("axisAligned_bbox_zDistance_Average_Normalized")
phenotype_timepoint = timepoint_two

# Initialize the multiple-QTL-model with the results from single QTL analysis 

init_qtl = makeqtl(cross_inputcross, 
                   chr=c(3,7,10),
                   pos=c(134.3552, 71.35577, 48.26723))

init_qtl_formula = y ~ Q1 + Q2 + Q3
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<#
#############################################################################################
