...
def returnPointXYZAndLabelbyRow():
    pointXYZAndLabelbyRow = []
    numAttributes = 0
    Xindex = 0
    Yindex = 0
    Zindex = 0
    for line in arffFile:
        ...
        if (line.find("@") < 0 and line.find("&") < 0):
            ...
            # The label is at the very end, index [-1]
            if (len(featureVector) > 1):
                pointXYZAndLabel = (featureVector[Xindex], featureVector[Yindex], featureVector[Zindex], featureVector[-1].rstrip())
                pointXYZAndLabelbyRow.append(pointXYZAndLabel)
    return pointXYZAndLabelbyRow
...
pointXYZAndLabelList = returnPointXYZAndLabelbyRow()

printPLYheader(len(pointXYZAndLabelList))
for i in range(len(pointXYZAndLabelList)):
    pointXYZAndLabel = pointXYZAndLabelList[i]
    sys.stdout.write(pointXYZAndLabel[0] + " " + pointXYZAndLabel[1] + " " + pointXYZAndLabel[2] + " ")
    if (pointXYZAndLabel[3] == "STEM"):
        sys.stdout.write("0 255 255 255\n")
    elif (pointXYZAndLabel[3] == "INFLORESCENCE"):
        sys.stdout.write("255 215 0 255\n")
    elif (pointXYZAndLabel[3]  == "LEAF"):
        sys.stdout.write("0 255 0 255\n")
    elif (pointXYZAndLabel[3] == "WHORL"):
        sys.stdout.write("0 0 255 255\n")
    elif (pointXYZAndLabel[3] == "JUNCTION"):
        sys.stdout.write("0 255 255 255\n")
    elif (pointXYZAndLabel[3] == "TIP"):
